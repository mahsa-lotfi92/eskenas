...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if Transaction.objects.filter(user=user_admin).count() == 0:
            ...
            Transaction.objects.create(bankAccount=account_kif, user = user_admin, date = baseDate + datetime.timedelta(days=0), Category = cat_sayer, isIncome = False, cost = 10 * 1000, description='خرج اضافی')
            Transaction.objects.create(bankAccount=account_kif, user = user_admin, date = baseDate + datetime.timedelta(days=0), Category = cat_sayer, isIncome = True, cost = 40 * 1000, description='پول توجیبی')
            ...
            Transaction.objects.create(bankAccount=account_kif, user = user_admin, date = baseDate + datetime.timedelta(days=4), Category = cat_huquq, isIncome = True, cost = 2 * 1000, description='')
